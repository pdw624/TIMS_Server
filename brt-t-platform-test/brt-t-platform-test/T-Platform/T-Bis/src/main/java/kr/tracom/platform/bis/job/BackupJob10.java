...
public class BackupJob10 implements Job {
	private final Logger logger = LoggerFactory.getLogger(this.getClass());
	private final String period = String.valueOf(CodeManager.ScheduleCycle.TEN_SECONDS.getValue());
	...
	private void run(String serviceId, SqlSessionFactory platformDb) {
		...
		for(Object obj : list) {
			...
			if(item.getType().equalsIgnoreCase(CodeManager.BackupType.MOVE.getValue())) {
				...
				backupArgs.setSelectWhere(item.getSelectWhere());
		...
		if(total > 0) {
			logger.debug("bis backup job(10) : " + sb.toString());
		}
	}
}
